...
function addRender(){
  ...
  taskArray.forEach(function(elem){
    li += `<li class="main-list--link">
            <div class="main-link">
                <input id='${elem.id}' class="main-input" type="checkbox"}>
                <span class="main-text">${elem.elem}</span>
            </div>
            <button id='${elem.id}' class="btn">X</button>
          </li>`;
...
const addChecked = (event) =>{
  const changeElem = event.target.closest('.main-input');
  console.log(changeElem);
  taskArray.forEach((task) => {
    if(task.id == id){
      changeElem.status == false;
      changeElem.status = !changeElem.status;
    }
  });
  addRender();
};

$ContainerTask.addEventListener('change', (event) =>{
  addChecked(event);
});
